...
의료 진단을 대체하지 않습니다. 문진 진행 중 상태는 브라우저의 React state에만 존재해 새로고침 시
소실됩니다(언제든 "처음으로" 버튼으로도 직접 초기화할 수 있습니다). 로그인한 회원이 문진을 완료하면
그 결과(처방전/녹음에서 추출한 텍스트, 문진기록, 종합소견)는 Neon Postgres에 영구 저장됩니다 —
원본 오디오/이미지 파일은 저장하지 않습니다. PWA로 지원되어 모바일/데스크톱 홈 화면에 설치할 수
있습니다.
